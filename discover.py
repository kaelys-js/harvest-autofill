...
def _nth_wd(y,mo,wd,n):
    d=datetime.date(y,mo,1); return d+datetime.timedelta(((wd-d.weekday())%7)+7*(n-1))
def _mon_before(y,mo,day):
    d=datetime.date(y,mo,day-1); return d-datetime.timedelta(d.weekday()%7)
def _last_wd(y,mo,wd):
    import calendar
    d=datetime.date(y,mo,calendar.monthrange(y,mo)[1]); return d-datetime.timedelta((d.weekday()-wd)%7)
...
def gen_holidays(region, years):
    ...
    r=region.upper(); out=set()
    for y in sorted(set(years)):
        ny=f"{y}-01-01"; canada=f"{y}-07-01"; xmas=f"{y}-12-25"; rem=f"{y}-11-11"
        gf=(_easter(y)-datetime.timedelta(2)).isoformat()
        family=_nth_wd(y,2,0,3).isoformat(); victoria=_mon_before(y,5,25).isoformat()
        labour=_nth_wd(y,9,0,1).isoformat(); thanks_ca=_nth_wd(y,10,0,2).isoformat()
        if r=="CA-BC":
            out|={ny,family,gf,victoria,canada,_nth_wd(y,8,0,1).isoformat(),labour,f"{y}-09-30",thanks_ca,rem,xmas}
        elif r=="CA-ON":
            out|={ny,family,gf,victoria,canada,labour,thanks_ca,xmas,f"{y}-12-26"}
        elif r=="CA-AB":
            out|={ny,family,gf,victoria,canada,_nth_wd(y,8,0,1).isoformat(),labour,thanks_ca,rem,xmas}
        elif r=="CA-QC":
            out|={ny,gf,victoria,f"{y}-06-24",canada,labour,thanks_ca,xmas}
        elif r=="US-FEDERAL":
            out|={ny,_nth_wd(y,1,0,3).isoformat(),_nth_wd(y,2,0,3).isoformat(),_last_wd(y,5,0).isoformat(),
                   f"{y}-06-19",f"{y}-07-04",labour,_nth_wd(y,10,0,2).isoformat(),rem,_nth_wd(y,11,3,4).isoformat(),xmas}
    return sorted(out)
...
def discover():
    out = {}
    hacc = os.environ.get("HARVEST_ACCOUNT_ID"); htok = os.environ.get("HARVEST_ACCESS_TOKEN")
    if hacc and htok:
        H = {"Authorization": f"Bearer {htok}", "Harvest-Account-Id": hacc, "User-Agent": "harvest-discover"}
        me = get("https://api.harvestapp.com/v2/users/me", H)
        out["harvest"] = {"user_id": me["id"], "user_agent": f"harvest-fill ({me.get('email','')})"}
        pa = get("https://api.harvestapp.com/v2/users/me/project_assignments?per_page=100", H)
        projects = {}
        for a in pa.get("project_assignments", []):
            p = a["project"]; tasks = {t["task"]["name"].lower(): t["task"]["id"] for t in a["task_assignments"]}
            projects[p["name"]] = {
                "project_id": p["id"], "client": a["client"]["name"],
    ...
    if ght:
        ...
        try:
            ...
            out["github"] = {"login": u["login"], "name": u.get("name"), "orgs": sorted(set(orgs))}
        except Exception as e:
            out["github"] = {"error": str(e)}
    # Azure DevOps repositories (only if PAT + org + project are set)
    pat, org, proj = os.environ.get("ADO_PAT"), os.environ.get("ADO_ORG"), os.environ.get("ADO_PROJECT")
    if pat and org and proj:
        import base64
        auth = "Basic " + base64.b64encode((":" + pat).encode()).decode()
        try:
            d = get(f"https://dev.azure.com/{org}/{proj}/_apis/git/repositories?api-version=7.1",
