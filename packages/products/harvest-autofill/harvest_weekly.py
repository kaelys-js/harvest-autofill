...
def vday(s):
    return tovan(parse_iso(s)).date()


def vdt(s):
    return tovan(parse_iso(s))
...
for e in cal["events"]:
    ...
    if e["allDay"] and any(k in e["title"].lower() for k in OOO):
        off.add(d)
    if e["status"] != "NO" and not e["allDay"] and is_social(e, dur):
        social_h[d] += dur
    if counts(e, dur):
        p = meeting_project(e["title"])
...
for org in GH_ORGS:
    ...
    for oid, (br, msg, date) in seen.items():
        dd = vday(date)
        if mon <= dd <= fri:
            day_events[dd].append((vdt(date), scope_of(msg, br)))
            gh_n += 1
...
def ado_events(a, b):
    ...
    if not (ado.get("enabled", True) and pat):
        return
    org = ado.get("org")
    ...
    if not (org and proj and author and repos):
        return
    hdr = {
    ...
    def add(dt_iso, cid):
        ...
        dd = vday(dt_iso)
        if a <= dd <= b:
            day_events[dd].append((vdt(dt_iso), bucket))
            return True
    ...
    for repo in repos:
        ...
        try:
            st, r = http(
                ...
                f"&searchCriteria.fromDate={a.isoformat()}&searchCriteria.toDate={(b + datetime.timedelta(1)).isoformat()}&api-version=7.1",
                hdr,
            )
        ...
        try:
            st, r = http(
                f"{base}/{repo}/pushes?searchCriteria.fromDate={a.isoformat()}"
                f"&searchCriteria.toDate={(b + datetime.timedelta(1)).isoformat()}&$top=200&api-version=7.1",
                hdr,
            )
...
ado_n = sum(
    1
    for d in day_events
    for (dt, p) in day_events[d]
    if p == CFG.get("azure_devops", {}).get("project_bucket", "Wheaton")
)
flags.append(
    f"per-day timeline split — GitHub {gh_n} commits, Azure DevOps {ado_n} commits/pushes (work-hours {WORK_START}:00–{WORK_END}:00)"
)
...
for d in days:
    for p, m in attribute(day_events.get(d, []), d).items():
        week_mins[p] += m
...
for d in days:
    ...
    for p in ORDER:
        if mt.get(p, 0) > 0:
            dur = mt[p] * 60
            entries.append((p, MTASK[p], "meeting", mt[p], cur, cur + dur))
            cur += dur
    for p in ORDER:
        if dev.get(p, 0) > 0:
            dur = dev[p] * 60
            entries.append((p, DTASK[p], "dev", dev[p], cur, cur + dur))
            cur += dur
...
# ---------- dedup pre-flight ----------
st, ex = http(
    f"https://api.harvestapp.com/v2/time_entries?user_id={USER}&from={mon}&to={fri}",
    HHEAD,
)
...
if n_ex not in (0, None) and os.environ.get("SKIP_DEDUP") != "1":
    ...
    write_json("dedup", f"{n_ex} entries already there — nothing written")
...
for p in posts:
    ...
    if (
        ...
        write_json("fail", "stopped mid-write — see latest.log")
...
for d, kind, entries in plan:
    ...
    if kind == "work":
        ...
        if abs(s - TARGET) > 0.01:
            ok = False
            issues.append(f"{dn}: recorded {s}h but expected {TARGET:g}h")
            print(f"VERIFY FAIL: {d} sums to {s}h, expected {TARGET:g}")
    else:
...
if ok:
    ...
    emit(
        "fail",
        "Harvest · Needs a look",
        "written but verification failed",
        "; ".join(issues) or "see latest.log",
    )
